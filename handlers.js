...
async function handleAddComponent(tab) {
    ...
    logger.info('Начало извлечения данных по локаторам', {
        tabId: tab.id
    });
    try {
        ...
        chrome.scripting.executeScript({
            ...
            chrome.scripting.executeScript({
                ...
                if (chrome.runtime.lastError || !results || !results[0]?.result) {
                    ...
                    logger.error('Ошибка извлечения компонента', {
                        error: errorMsg
                    });
                ...
                const {
                    [MenuManager.STORAGE_KEY]: components = []
                } = await chrome.storage.local.get(MenuManager.STORAGE_KEY);
...
async function handleDeleteComponent(menuItemId) {
    ...
    logger.info('Запрос на удаление компонента', {
        componentId
    });
    try {
        const {
        ...
        await chrome.storage.local.set({
            [MenuManager.STORAGE_KEY]: updatedComponents
        });
        await menuManager.refreshMenu();
        logger.info('Компонент удален из хранилища', {
            componentId
        });
    } catch (ex) {
...
async function handleCopyComponent(menuItemId, tab) {
    try {
        ...
        if (!foundComponent) {
            logger.error('Сохраненный компонент не найден', {
                componentId: menuItemId
            });
            UIManager.showError(tab.id, 'Ошибка: компонент не найден', 4000, true);
            return;
        }
        logger.info('Копирование JSON компонента', {
            componentName: foundComponent.name
        });
...
async function handleLoadOffer(menuItemId, tab) {
    try {
        const {
            savedOffers = {}
        } = await chrome.storage.local.get('savedOffers');
        const savedOffer = savedOffers[menuItemId];
        if (!savedOffer) {
            logger.error('Сохраненное предложение не найдено', {
                offerId: menuItemId
            });
            UIManager.showError(tab.id, 'Ошибка: предложение не найдено', 4000, true);
            return;
        }
        logger.info('Загрузка сохраненного предложения', {
            offerName: savedOffer.name
        });
...
async function handleGenerateOffer(tab, language = null) {
    ...
    try {
        ...
        } = await chrome.storage.sync.get('geminiModel');
        ...
        if (language) {
            previewUrl += `?lang=${language}`;
        }
        logger.info(`Открываем URL: ${previewUrl}`);
        ...
        if (existingTabs.length > 0) {
            ...
            logger.info('Вкладка preview-offer обновлена и активирована', {
                tabId: targetTab.id
            });
        } else {
        ...
        logger.error('Ошибка открытия вкладки предложения', {
            error: ex.message,
            stack: ex.stack
        });
